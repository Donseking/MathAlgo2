...
# 初始化日誌管理器
logger_manager = Logger(name="Math_mode", log_file=str(log_file), level=logging.INFO)
...
class Calculus:
    ...
    Attributes:
        expression (str): 數學表達式

    Methods:
        derivative(): 計算導數
        definite_integral(a, b): 計算定積分
    """
    ...
    def __init__(self, expression: str):
        ...
        Args:
            expression (str): 數學表達式，例如 "x^2 + 2*x + 1"

        Raises:
            ValueError: 表達式格式無效時拋出
        """
        self.expression = expression
        self.logger = logger_manager
        try:
            self.func_expr = (
                sp.sympify(expression) if isinstance(expression, str) else expression
            )
    ...
    def definite_integral(self, lower: float, upper: float) -> float:
        ...
        * 定積分值
        ...
        try:
            ...
            result = np.trapezoid(y_vals, x_vals)
    ...
    def limit(self, point: float, direction: Optional[str] = None) -> float:
        ...
        * 極限值
        ...
        try:
            ...
            self.logger.info(
                f"極限計算在 x -> {point} ({direction if direction else 'both'}) 的值為 {result}"
            )
            return result
        except Exception as e:
    ...
    def taylor_series(self, point: float, order: int) -> "Calculus":
        ...
        * 泰勒級數
        """
        try:
            result = sp.series(self.func_expr, x, int(point), order).removeO()
    ...
    def find_critical_points(self) -> List[float]:
        ...
        * 臨界點列表
        ...
        try:
            ...
            critical_points = [
                float(point.evalf()) for point in critical_points if point.is_real
            ]
...
class Matrix:
    ...
    def __init__(self, data):
        ...
        try:
            if isinstance(data, str):
                ...
                self.logger.info(
                    f"矩陣初始化成功，維度為 {converted_data.shape[0]}x{converted_data.shape[1]}"
                )
            else:
                # 檢查資料維度
    ...
    def determinant(self) -> float:
        ...
        determinant_value = _det_recursive(self.data.tolist())  # type: ignore
...
__all__ = ["Calculus", "Matrix", "Vector_space"]
