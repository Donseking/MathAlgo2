...
class Logger:
    ...
    def _create_formatter(
        ...
        return logging.Formatter(
            format_string or self.format_string, datefmt="%Y-%m-%d %H:%M:%S"
        )

    def _setup_handler(
        self, handler: logging.Handler, format_string: Optional[str] = None
    ) -> logging.Handler:
        """
        設置處理器的格式化器和級別
    ...
    def log_exception(
        self, exc: Exception, additional_info: Optional[Dict] = None
    ) -> None:
        """
        記錄異常信息及可選的附加上下文
