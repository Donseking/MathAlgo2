...
Functions:
    infix_to_postfix: 中綴表達式轉後綴
    evaluate_postfix: 計算後綴表達式
    prime_factorization: 質因數分解

Examples:
    >>> from mathalgo2.MathUtiles import MathUtils
    >>> MathUtils.prime_factorization(12)
    [2, 2, 3]
"""
...
# 初始化日誌管理器
logger_manager = Logger(name="math_utiles", log_file=str(log_file), level=logging.INFO)
...
class MathUtils:
        ...
        while i < len(expression):
            ...
            # 如果是運算符
                ...
                while (
                    stack
                    and stack[-1] != "("
                    and MathUtils.precedence(stack[-1]) >= MathUtils.precedence(char)
                ):
                    output.append(stack.pop())
                stack.append(char)
    ...
    def prime_factorization(n: int) -> List[int]:
        ...
        Raises:
            ValueError: 輸入不是正整數時拋出

        Examples:
            >>> MathUtils.prime_factorization(12)
            [2, 2, 3]
            >>> MathUtils.prime_factorization(17)
            [17]
        """
    ...
    def convert_to_matrix(data: str) -> np.ndarray:
        ...
        try:
            ...
            logger_manager.debug(f"轉換為整數列表: {data_list}")

            data_array = np.array(data_list)
            logger_manager.debug(f"轉換為numpy數組: {data_array}")

            size = data_array.size
            logger_manager.info(f"數組大小: {size}")

            factors = MathUtils.factorization(size)
            logger_manager.debug(f"因數分解結果: {factors}")

            middle_index = len(factors) // 2
            matrix_size = factors[middle_index]
            logger_manager.info(f"選擇的矩陣維度: {matrix_size}")

            reshaped_data = data_array.reshape(matrix_size[0], matrix_size[1])
    ...
    def catalan_number(n: int) -> int:
        ...
        try:
            ...
            logger_manager.debug(
                f"計算過程: C(2*{n},{n})/{n+1} = {numerator}/{n+1} = {result}"
            )
