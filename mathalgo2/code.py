...
# 初始化日誌管理器
logger_manager = Logger(name="Code", log_file=str(log_file), level=logging.INFO)
...
class CodeBase:
    ...
    def morse_encode(text: str) -> str:
        ...
        try:
            result = " ".join(
                CodeMaps.MORSE_CODE.get(char, char) for char in text.upper()
            )
    ...
    def ascii_decode(ascii_str: str) -> str:
        ...
        try:
            result = "".join(
                chr(int(code)) for code in ascii_str.split() if code.strip()
            )
...
class ClassicalCipher:
    ...
    def vigenere_decode(encrypted_text: str, key: str) -> str:
        ...
        try:
            ...
            # 生成完整長度的金鑰
            full_key = (key * (len(encrypted_text) // len(key) + 1))[
                : len(encrypted_text)
            ]
...
class ModernCipher:
    ...
    def rsa_encrypt(message: str, public_key: bytes, show_progress: bool = True) -> str:
        ...
        try:
            ...
            logger_manager.info(f"RSA 加密完成，原始長度：{len(message)}，" f"加密後長度：{len(result)}")
            return result
    ...
    def rsa_decrypt(encrypted_message: str, private_key: bytes) -> str:
        ...
        try:
            ...
            logger_manager.info(
                f"RSA解密成功: {encrypted_message[:20]}... -> {result[:20]}..."
            )
            return result
        except Exception as e:
    ...
    @staticmethod
    def aes_encrypt(
        message: str, key: bytes = None, show_progress: bool = True
    ) -> tuple:
        """
        # AES 加密
    ...
    def aes_decrypt(encrypted_message: str, key: bytes) -> str:
        ...
        try:
            ...
            logger_manager.info(
                f"AES解密成功: {encrypted_message[:20]}... -> {result[:20]}..."
            )
            return result
        except Exception as e:
    ...
    def des_decrypt(encrypted_message: str, key: bytes) -> str:
        ...
        try:
            ...
            result = decrypted.decode()
            logger_manager.info(
                f"DES解密成功: {encrypted_message[:20]}... -> {result[:20]}..."
            )
            return result
        except Exception as e:
    ...
    def rsa_encrypt_file(
        input_file: str, public_key: bytes, output_file: str = None
    ) -> str:
        """
        # RSA 檔案加密
    ...
    def des_encrypt_file(
        input_file: str, key: bytes = None, output_file: str = None
    ) -> tuple:
        """
        # DES 檔案加密
...
__all__ = ["CodeBase", "ClassicalCipher", "ModernCipher"]
