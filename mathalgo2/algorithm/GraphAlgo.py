...
class GraphAlgo(Algorithm):
    ...
    def __init__(
        ...
        self.graph = graph
        self.weights = (
            weights if weights else {(u, v): 1.0 for u in graph for v in graph[u]}
        )
        self.colors = {}  # 節點顏色映射
        ...
        try:
            ...
            while pq:
                ...
                for neighbor in self.graph[current]:
                    ...
                    weight = self.weights.get((current, neighbor)) or self.weights.get(
                        (neighbor, current), 1.0
                    )
    ...
    def dfs(
        self, start: Any, callback: Optional[Callable[[Any], None]] = None
    ) -> List[Any]:
        """深度優先搜尋演算法
    ...
    def bfs(
        ...
        try:
            ...
            while queue:
                ...
                self.colors[vertex] = self.node_colors["visiting"]
                self.logger.debug(
                    f"將節點 {vertex} 標記為正在訪問狀態 (顏色: {self.node_colors['visiting']})"
                )
                ...
                self.colors[vertex] = self.node_colors["visited"]
                self.logger.debug(
                    f"將節點 {vertex} 標記為已訪問狀態 (顏色: {self.node_colors['visited']})"
                )
    ...
    def _update_graph_plot(self):
        ...
        try:
            ...
            # 添加邊權重標籤
            edge_labels = {
                (u, v): self.weights.get((u, v)) or self.weights.get((v, u), 1.0)
                for u in self.graph
