...
# 初始化日誌管理器
logger_manager = Logger(name="SortAlgo", log_file=str(log_file), level=logging.INFO)
...
class Sorting(Algorithm):
    ...
    def __init__(self, arr: List[Any], animation_speed: float = 0.1):
        ...
        # 視覺化顏色配置
        self.colors = {"default": "skyblue", "highlight": "red", "sorted": "green"}
    ...
    def _partition(self, low: int, high: int, reverse: bool) -> int:
        ...
        # 最後一次視覺化更新
        if self.fig is not None:
            self._update_plot(highlight_indices=[i + 1], status=f"Placed pivot={pivot}")
    ...
    def _merge_sort_helper(self, arr: List[Any], reverse: bool) -> List[Any]:
        ...
        if len(arr) > 1:
            ...
            # 更新視覺化
            if self.fig is not None:
                self._update_plot(highlight_indices=[], status=f"合併: {merged}")
    ...
    def animate(self, algorithm: str = "bubble", reverse: bool = False):
        ...
        self.bars = self.ax.bar(
            range(len(self.arr)), self.arr, color=self.colors["default"]
        )
        ...
        if algorithm == "bubble":
            self.anim = FuncAnimation(
                self.fig,
                self._bubble_sort_animation,
                frames=len(self.arr),
                repeat=False,
            )
        elif algorithm == "quick":
            self.anim = FuncAnimation(
                self.fig, self._quick_sort_animation, frames=len(self.arr), repeat=False
            )
        elif algorithm == "insertion":
            self.anim = FuncAnimation(
                self.fig,
                self._insertion_sort_animation,
                frames=len(self.arr),
                repeat=False,
            )
        elif algorithm == "merge":
            self.anim = FuncAnimation(
                self.fig, self._merge_sort_animation, frames=len(self.arr), repeat=False
            )
        else:
            self.logger.error(f"不支持的算法: {algorithm}")
            return
