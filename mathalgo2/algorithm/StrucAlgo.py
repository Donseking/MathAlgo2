...
# 初始化日誌管理器
logger_manager = Logger(name="StrucAlgo", log_file=str(log_file), level=logging.INFO)
...
class BaseTree(BaseAlgo, Generic[T]):
    ...
    def visualize(self, filename: str = "tree"):
        ...
        def add_to_graph(
            node: Optional[BaseTree.Node], x: float = 0, y: float = 0, layer: int = 0
        ):
            if node:
    ...
    def create_animation(
        self, operations: List[Tuple[str, Any]], filename: str = "tree_animation.gif"
    ):
        """創建樹操作的動畫
        ...
        ani = animation.FuncAnimation(
            fig, update, frames=len(operations), interval=1000, blit=True
        )
...
class BinaryTree(BaseTree):
    ...
    def _delete_recursive(
        self, node: Optional[BaseTree.Node], value: T
    ) -> Optional[BaseTree.Node]:
        """遞歸刪除節點
...
class Heap(BaseAlgo):
    ...
    def _heapify_up(self, index: int):
        ...
        while index > 0 and self.heap[self.parent(index)] < self.heap[index]:
            self.logger.debug(
                f"交換元素 {self.heap[index]} 和父節點 {self.heap[self.parent(index)]}"
            )
...
__all__ = ["BinaryTree", "AVLTree", "UnionFind", "Heap", "DataStructureFactory"]
